...
# ROI 区域 [x1, y1, x2, y2]（原始图像坐标）
ROI = [100, 100, 540, 540]   # 请根据实际画面调整
OCCUPANCY_RATIO = 0.7        # 面积占比阈值
...
def draw(image, boxes, scores, classes, ratio, padding):
    ...
    for box, score, cl in zip(boxes, scores, classes):
        ...
        # 计算检测框与ROI的交集面积
        inter_x1 = max(top, roi_x1)
        inter_y1 = max(left, roi_y1)
        inter_x2 = min(right, roi_x2)
        inter_y2 = min(bottom, roi_y2)
        ...
        # 绘制所有检测框（或只绘制与ROI有交集的，此处绘制全部以作参考）
    ...
    # 判断空位状态
    occupied = max_overlap_ratio >= OCCUPANCY_RATIO
    status_text = "OCCUPIED" if occupied else "EMPTY"
    color = (0, 0, 255) if occupied else (0, 255, 0)

    # 在图像左上角显示状态和占比
    cv2.putText(image, f"Status: {status_text} ({max_overlap_ratio:.2f})", (50, 50),
